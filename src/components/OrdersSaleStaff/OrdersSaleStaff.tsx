...
import { clearLocalhostStorage } from "@/services/localstorage.services"
import { recordAttendance } from "@/services/attendance.services"
import ReceiptDeliveryModal from "../ReceiptDelivery/ReceiptDeliveryModal"
...
const OrdersSaleStaff = () => {
  ...
  const [filteredOrders, setFilteredOrders] = useState<Record<string, Order[]>>({})
  const [isRefreshing, setIsRefreshing] = useState(false)
  const [receiptDeliveryModalOpen, setReceiptDeliveryModalOpen] = useState(false)
  ...
  useEffect(() => {
    const fetchAndFilterOrders = async () => {
      try {
        ...
        const filtered = Object.entries(ordersByStatus).reduce(
          (acc, [status, orders]) => {
            const filteredOrders = (orders as Order[]).filter((order) => !paidOrderIds.includes(order.id))
            if (filteredOrders.length > 0) acc[status] = filteredOrders
            return acc
  ...
  // Apply additional filters
  const applyFilters = (orders: Record<string, Order[]>) => {
    return Object.entries(orders).reduce(
      (acc, [status, ordersList]) => {
        const filtered = ordersList.filter((order) => {
          // Search term filter
          const matchesSearch =
            searchTerm === "" ||
            ...
            order.fullname?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            order.phone?.includes(searchTerm) ||
            order.cart?.some((item: any) => item.name.toLowerCase().includes(searchTerm.toLowerCase()))
          ...
          // Order type filter
          const matchesOrderType =
            orderTypeFilter === "all" || order.orderType?.toLowerCase() === orderTypeFilter.toLowerCase()
  ...
  const handleOrderActionAPI = async (paymentData: any) => {
    ...
    try {
      ...
      await fetchHistoricalOrders() // Actualizar la lista de órdenes
      ...
      // Set the processed order and open the receipt delivery modal
      setProcessedOrder({ ...order, payment: paymentSummary })
      setReceiptDeliveryModalOpen(true)
    } catch (error) {
      console.error("Error en handleOrderActionAPI:", error)
    }
  }
      ...
      >
        ...
        >
          <Toolbar variant="dense" sx={{ minHeight: isMobile ? 48 : 56 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
              {isMobile &&
                Object.values(ordersByStatus).reduce((sum, orders) => sum + (orders?.length || 0), 0) > 0 && (
                  <motion.div whileHover={{ scale: 1.1 }}>
                    <Badge
                      badgeContent={Object.values(ordersByStatus).reduce(
                        (sum, orders) => sum + (orders?.length || 0),
                        0,
                      )}
                      color="primary"
                      sx={{ mr: 0.5 }}
        ...
        >
          <motion.div initial="hidden" animate="visible" variants={fadeIn} style={{ height: "100%" }} layout>
            <Paper
              elevation={isMobile ? 1 : 3}
              sx={{
                height: "100%",
                ...
                <Box sx={{ p: { xs: 1, sm: 2 } }}>
                  ...
                  {(searchTerm || statusFilter !== "all" || orderTypeFilter !== "all") && (
                    ...
                    >
                      ...
                      {searchTerm && (
                        <Chip size="small" label={`Buscar: "${searchTerm}"`} onDelete={() => setSearchTerm("")} />
                      )}
                ...
                <AnimatePresence>
                  {error && (
                    <motion.div
                      ...
                      transition={{ duration: 0.3 }}
                    >
                      <Alert severity="error" onClose={() => setError(null)} sx={{ mb: 1 }} component={motion.div}>
                        {error}
                      </Alert>
                    </motion.div>
                  )}
        ...
        {processedOrder && (
          <ReceiptDeliveryModal
            open={receiptDeliveryModalOpen}
            onClose={() => setReceiptDeliveryModalOpen(false)}
