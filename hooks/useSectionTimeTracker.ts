...
//       }
//     },
//     [userData, sectionTimes, totalTime, history, demos, namecompanies],
//   );
...
//   const [totalTime, setTotalTime] = useState<number>(0);
//   const [history, setHistory] = useState<{ section: string; timeSpent: number; startTime: number; endTime: number }[]>([]);
//   const [clicks, setClicks] = useState<{ section: string; element: string; timestamp: number }[]>([]); // Nuevo estado para clics
//   const [demos, setDemos] = useState<boolean>(false);
...
//       setHistory((prev) => [
//         ...prev,
//         { section, timeSpent: elapsedTime, startTime: startTimeRef.current!, endTime: now },
//       ]);
...
//       sendIntervalRef.current = setTimeout(() => {
//         if (isUserActiveRef.current && activeSectionRef.current && startTimeRef.current) {
//           handleSectionLeave(activeSectionRef.current);
//           sendData();
//         }
//         scheduleSend();
//       }, 10000); // Intervalo de 10 segundos
